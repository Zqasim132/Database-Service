...
{
    internal class DBComponentDapper_1
    {
        public readonly SqlConnection connection;
        private string routine = "DBC_Dapper_1";
        public delegate void QueriesExecutedHandler(string name);
        public event QueriesExecutedHandler QueriesExecuted;
        public DBComponentDapper_1(IConfigurationRoot _configuration)
        {
        ...
        public bool InsertToDatabase(ContactSummaryDetails contactSummaryDetails, out long time)
        {
            ...
            try
            {
                ...
                var rows = connection.Execute(query, Parameters);
        ...
        private bool IDChecker(Guid? id)
        {
            ...
            var reader = connection.QueryFirst(query, parameters);
        ...
        public bool Read(string client, out long time)
        {
            try
            {
                ...
                var query = "select select TOP(1000) * from Contact_Summary_Details";
                ...
                Console.WriteLine($"Read time of First client: {client} is: {time} milliseconds ");
